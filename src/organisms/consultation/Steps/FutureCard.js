...
export const FutureCard = (props) => {
    const { selectedConditions, selectedTemes, setFutureData, futureData } = props
    ...
    const handleChangeIde = ({ target, indexFaixaItem, keyIde }) => {
        setFutureData((prevFutureData) => {
            const novoTema = prevFutureData?.temas?.map((item, indexFaixa) => {
            ...
            return { ...prevFutureData, temas: novoTema }
        })
    }
    ...
    const handleChangeMarks = ({ type, field, index, keyId }) => {
        setFutureData((prevFutureData) => {
            const newFaixaTema = prevFutureData?.temas?.map((item, indexFaixa) => {
            ...
            return { ...prevFutureData, temas: newFaixaTema }
        })
    }
        ...
        <Box sx={{ display: 'flex', padding: '15px', backgroundColor: '#fff', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 3, padding: '0px 10px', flexDirection: 'column' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexDirection: 'column' }}>
                    {futureData?.temas?.map((item, index) => {
